...
def register_if_improved(model: nn.Module, accuracy: float, min_threshold: float = 0.55) -> str:
    ...
    if accuracy > min_threshold:
        print(f"🏆 Model passed threshold ({accuracy:.2%}). Promoting to Model Registry!")
        ...
        print(f"❌ Model failed threshold ({accuracy:.2%}, required: {min_threshold:.2%}). Dropping.")
